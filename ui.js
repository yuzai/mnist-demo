...
export function setLoadButtonCallback(callback) {
...
export function setPredictButtonCallback(callback) {
...
export function setCanvasInput(activateDraw, deactivateDrawAndPredict, draw, clear){
  const canvas = document.getElementById('input-canvas');
  canvas.addEventListener('mousedown', activateDraw);
  canvas.addEventListener('mouseup', deactivateDrawAndPredict);
  canvas.addEventListener('mouseleave', deactivateDrawAndPredict);
  canvas.addEventListener('mousemove', draw);
  document.getElementById('clear').addEventListener('click', clear);
}

export function updatePredict(data) {
  const lists = document.getElementsByClassName('predict-result-bar');
  const datalists = document.getElementsByClassName('predict-result-data');
  
  for(let i = 0;i < data.length; i++){
    lists[i].style.width = `${300*data[i]}px`;
    datalists[i].innerHTML = `${i} :     ${data[i]}`;
  }
}